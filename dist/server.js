...
class VideoEnhancementMCPServer {
    ...
    setupTools() {
        // create_task tool
        this.server.tool('create_task', `创建视频增强任务（异步）
...
- type: "url" 或 "local"
- resolution: 目标分辨率`, CreateTaskSchema.shape, async (args) => {
            try {
                const result = await this.createTask(args.video_source, args.type, args.resolution);
                return {
                    content: [
                        {
                            type: 'text',
        ...
        this.server.tool('get_task_status', '查询视频增强任务状态', GetTaskStatusSchema.shape, async (args) => {
            try {
        ...
        this.server.tool('enhance_video_sync', `同步增强视频（阻塞等待完成）
...
- timeout: 超时时间（秒）`, EnhanceVideoSyncSchema.shape, async (args) => {
            try {
                const result = await this.enhanceVideoSync(args.video_source, args.type, args.resolution, args.poll_interval, args.timeout);
                return {
                    content: [
                        {
                            type: 'text',
    ...
    async createTask(videoSource, sourceType, resolution) {
        let contentItem;
        if (sourceType === 'local') {
            // 本地上传：读取文件转 base64
    ...
    async enhanceVideoSync(videoSource, sourceType, resolution, pollInterval, timeout) {
        // 创建任务
